...
├── bot/
│   ├── notifier.js         optional WeChat notifier (wechaty) for trim-complete alerts
│   └── package.json
...
## Optional: get a WeChat message when a trim finishes

`bot/notifier.js` runs a small [wechaty](https://wechaty.js.org) bot that
sends a WeChat message to a contact or room once a trim job completes.
...
```bash
export NOTIFIER_URL=http://localhost:8788
uvicorn backend.main:app --reload --port 8000
```

With `NOTIFIER_URL` unset (the default), the backend skips notifications
entirely — trimming works exactly as before.
